<template>
  <div class="px-gutter max-w-container-max mx-auto" style="padding-top: 6rem; padding-bottom: 3rem;">
    <div class="mb-stack-lg">
      <h2 class="font-display text-headline-lg text-primary">Manajemen Tahun Ajaran</h2>
      <p class="text-on-surface-variant text-body-md">Kelola tahun ajaran dan semester untuk kegiatan akademik.</p>
    </div>
    ...
    <div class="glass-card rounded-xl shadow-sm overflow-hidden">
      <div class="p-stack-md border-b border-outline-variant/20 flex flex-wrap items-center justify-between gap-stack-md">
        <h3 class="font-display text-title-lg text-primary">Daftar Tahun Ajaran</h3>
        <button class="flex items-center gap-2 px-4 py-2 bg-primary text-on-primary rounded-lg text-label-md hover:brightness-110 transition-all shadow-sm" @click="openAddModal">
          <span class="material-symbols-outlined text-sm">add</span> Tambah Tahun Ajaran
        </button>
      </div>
        ...
        <table class="w-full text-left">
          <thead class="bg-surface-container-low">
            <tr>
              ...
              <th class="px-6 py-4 text-label-md text-on-surface-variant">Tahun Ajaran</th>
              <th class="px-6 py-4 text-label-md text-on-surface-variant">Semester</th>
          ...
          <tbody class="divide-y divide-outline-variant/10">
            <tr v-for="item in items" :key="item.id" class="hover:bg-primary-fixed/5 transition-colors">
              ...
              <td class="px-6 py-4">
                <span class="bg-surface-container-low px-2 py-1 rounded text-label-sm text-on-surface-variant">{{ item.semester }}</span>
              </td>
            ...
            <tr v-if="items.length === 0">
              <td colspan="99"  class="px-6 py-12 text-center text-on-surface-variant text-label-md">Belum ada data tahun ajaran. Klik "Tambah Tahun Ajaran" untuk menambahkan.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/40 backdrop-blur-sm" @click.self="closeModal">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-modal-enter">
          <div class="p-6 border-b border-outline-variant/20 flex justify-between items-center">
            <h3 class="font-display text-title-lg text-primary">{{ isEditing ? 'Edit Tahun Ajaran' : 'Tambah Tahun Ajaran' }}</h3>
          ...
          <div class="p-6 space-y-4">
            <div>
              <label class="text-label-sm text-on-surface-variant block mb-1">Nama Tahun Ajaran</label>
              <input v-model="form.name" type="text" class="w-full bg-surface-container-low border-none rounded-lg text-label-md py-2.5 px-3 focus:ring-primary" placeholder="Contoh: 2024/2025" />
            </div>
            <div>
              <label class="text-label-sm text-on-surface-variant block mb-1">Semester</label>
              <select v-model="form.semester" class="w-full bg-surface-container-low border-none rounded-lg text-label-md py-2.5 px-3 focus:ring-primary">
      ...
      <div v-if="showDeleteModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/40 backdrop-blur-sm" @click.self="showDeleteModal = false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-modal-enter">
          <div class="p-6 text-center">
            ...
            <h3 class="font-display text-title-lg text-on-surface mb-2">Hapus Tahun Ajaran</h3>
            <p class="text-on-surface-variant text-label-md mb-1">Apakah Anda yakin ingin menghapus tahun ajaran berikut?</p>
            <p class="font-bold text-on-surface text-body-md">{{ deleteTarget?.name }} - {{ deleteTarget?.semester }}</p>
          </div>
          <div class="p-6 border-t border-outline-variant/20 flex justify-end gap-3">
...
async function fetchData() {
  ...
  error.value = ''
  try {
    const data = await $fetch('/api/master-data/academic-years')
    items.value = data || []
  } catch (e: any) {
...
async function doDelete() {
  ...
  try {
    await $fetch(`/api/master-data/academic-years/${deleteTarget.value.id}`, { method: 'DELETE' })
...
function confirmDelete(item: AcademicYear) {
...
async function bulkDelete() {
  ...
  try {
    await Promise.all(selected.value.map(id => $fetch(`/api/master-data/academic-years/${id}`, { method: 'DELETE' })))
    clearSelection()
    await fetchData()
  } catch (e: any) {
    error.value = e.message || 'Gagal menghapus'
