...
const { selected, allSelected, toggleAll, toggleOne, isSelected, clearSelection, selectedCount } = useTableSelection(() => filteredResults)
...
async function bulkDelete() {
  ...
  try {
    await Promise.all(selected.value.map(id => $fetch(`/api/psb/results/${id}`, { method: 'DELETE' })))
    clearSelection()
    await fetchData()
  } catch (e: any) {
    error.value = e.message || 'Gagal menghapus'
