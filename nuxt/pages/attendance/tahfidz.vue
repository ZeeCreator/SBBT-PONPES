...
const STATUS_OPTIONS = [
  { value: 'setor', label: 'S' },
  { value: 'tidak_setor', label: 'TS' },
  { value: 'alpa', label: 'A' },
]
const DEFAULT_STATUS = 'setor'

const SUMMARY_CONFIG = [
  { key: 'setor', label: 'Setor', bg: 'bg-green-50', labelColor: 'text-green-700', valueColor: 'text-green-600', barColor: 'bg-green-500' },
  { key: 'tidak_setor', label: 'Tidak Setor', bg: 'bg-amber-50', labelColor: 'text-amber-700', valueColor: 'text-amber-600', barColor: 'bg-amber-500' },
...
async function doDelete() {
  ...
  try {
    const token = await getIdToken()
    const res = await fetch(`/api/attendance/tahfidz/${editingId.value}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    })
    if (res.ok) {
...
function statusClass(val: string) {
  const map: Record<string, string> = {
    setor: 'border-green-300 text-green-700',
    tidak_setor: 'border-amber-300 text-amber-700',
