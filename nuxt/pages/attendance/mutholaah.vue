  ...
  <div class="px-gutter max-w-container-max mx-auto" style="padding-top: 6rem; padding-bottom: 3rem;">
    ...
    <div class="flex items-center justify-between mb-stack-lg flex-wrap gap-4">
      <div>
        ...
        <p class="text-on-surface-variant text-body-md">Catat dan kelola kehadiran muthola'ah santri per bulan</p>
      </div>
    ...
    <div v-if="Object.keys(attendanceData).length > 0" class="glass-card rounded-xl shadow-sm overflow-hidden mb-stack-lg">
      <div class="p-stack-md border-b border-outline-variant/20">
        <h3 class="font-display text-title-lg text-primary flex items-center gap-2">
          <span class="material-symbols-outlined">bar_chart</span> Rekap Absensi Muthola'ah {{ selectedClass }}
...
async function loadSession() {
  ...
  try {
    const token = await getIdToken()
    const res = await fetch(`/api/attendance/mutholaah?month=${encodeURIComponent(selectedMonth.value)}&class=${encodeURIComponent(selectedClass.value)}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    if (!res.ok) {
...
function exportExcel() {
  ...
  a.href = url
  a.download = `Absensi_Mutholaah_${selectedClass.value}_${selectedMonth.value}.xls`
  a.click()
...
<html><head><title>Absensi Muthola'ah - ${selectedClass.value}</title>
<style>
...
<div class="header">
  <div class="header-left">INV. DINIYAH/ABSEN MUTHOLA'AH/KELAS ${selectedClass.value}</div>
