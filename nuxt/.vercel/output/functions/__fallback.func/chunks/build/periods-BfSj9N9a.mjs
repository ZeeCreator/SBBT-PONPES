...
import { ssrRenderAttrs, ssrRenderList, ssrInterpolate, ssrRenderClass, ssrRenderTeleport, ssrRenderAttr } from 'vue/server-renderer';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))}><div class="mb-stack-lg"><h2 class="font-display text-headline-lg text-primary">Manajemen Periode</h2><p class="text-on-surface-variant text-body-md">Kelola periode SPP dan tahfidz untuk pembayaran dan kegiatan.</p></div><div class="grid grid-cols-1 md:grid-cols-4 gap-gutter mb-stack-lg"><!--[-->`);
      ...
      _push(`<!--]--></div><div class="glass-card rounded-xl shadow-sm overflow-hidden"><div class="p-stack-md border-b border-outline-variant/20 flex flex-wrap items-center justify-between gap-stack-md"><h3 class="font-display text-title-lg text-primary">Daftar Periode</h3><button class="flex items-center gap-2 px-4 py-2 bg-primary text-on-primary rounded-lg text-label-md hover:brightness-110 transition-all shadow-sm"><span class="material-symbols-outlined text-sm">add</span> Tambah Periode </button></div>`);
      if (unref(loading)) {
        ...
        _push(`<div class="overflow-x-auto"><table class="w-full text-left"><thead class="bg-surface-container-low"><tr><th class="px-6 py-4 text-label-md text-on-surface-variant">Nama Periode</th><th class="px-6 py-4 text-label-md text-on-surface-variant">Mulai</th><th class="px-6 py-4 text-label-md text-on-surface-variant">Selesai</th><th class="px-6 py-4 text-label-md text-on-surface-variant">Status</th><th class="px-6 py-4 text-label-md text-on-surface-variant">Aksi</th></tr></thead><tbody class="divide-y divide-outline-variant/10"><!--[-->`);
        ssrRenderList(unref(items), (item) => {
          _push(`<tr class="hover:bg-primary-fixed/5 transition-colors"><td class="px-6 py-4 text-label-md text-on-surface font-medium">${ssrInterpolate(item.name)}</td><td class="px-6 py-4 text-label-sm text-on-surface-variant">${ssrInterpolate(item.startDate)}</td><td class="px-6 py-4 text-label-sm text-on-surface-variant">${ssrInterpolate(item.endDate)}</td><td class="px-6 py-4"><button class="${ssrRenderClass(["px-3 py-1 text-[11px] font-bold rounded-full uppercase tracking-wider border-0 cursor-pointer transition-all", item.isActive ? "bg-primary-fixed text-on-primary-fixed" : "bg-surface-container text-on-surface-variant"])}">${ssrInterpolate(item.isActive ? "Aktif" : "Nonaktif")}</button></td><td class="px-6 py-4"><div class="flex items-center gap-2"><button class="p-1.5 rounded-lg hover:bg-primary-fixed/20 text-primary transition-colors" title="Edit"><span class="material-symbols-outlined text-sm">edit</span></button><button class="p-1.5 rounded-lg hover:bg-red-50 text-red-600 transition-colors" title="Hapus"><span class="material-symbols-outlined text-sm">delete</span></button></div></td></tr>`);
        });
        _push(`<!--]-->`);
        if (unref(items).length === 0) {
          _push(`<tr><td colspan="5" class="px-6 py-12 text-center text-on-surface-variant text-label-md">Belum ada data periode. Klik &quot;Tambah Periode&quot; untuk menambahkan.</td></tr>`);
        } else {
          _push(`<!---->`);
        }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/master-data/periods.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=periods-BfSj9N9a.mjs.map
