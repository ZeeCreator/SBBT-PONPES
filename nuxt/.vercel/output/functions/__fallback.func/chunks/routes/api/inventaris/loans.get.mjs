...
const loans_get = defineEventHandler(async (event) => {
  const query = getQuery(event);
  let items = await rtdbGetList("inventaris/loans");
  if (query.status) items = items.filter((i) => i.status === query.status);
  if (query.studentId) items = items.filter((i) => i.studentId === query.studentId);
  return items;
});

export { loans_get as default };
//# sourceMappingURL=loans.get.mjs.map
