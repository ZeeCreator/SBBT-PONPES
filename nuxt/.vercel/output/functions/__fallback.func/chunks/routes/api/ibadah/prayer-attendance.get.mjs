...
const prayerAttendance_get = defineEventHandler(async (event) => {
  const query = getQuery(event);
  let items = await rtdbGetList("prayerAttendance");
  if (query.studentId) items = items.filter((i) => i.studentId === query.studentId);
  if (query.date) items = items.filter((i) => i.date === query.date);
  return items;
});

export { prayerAttendance_get as default };
//# sourceMappingURL=prayer-attendance.get.mjs.map
